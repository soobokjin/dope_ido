...
interface IStake {
    ...
    ) external pure returns (bytes memory); // noooo
...
contract Stake is IStake, Operator, Initializable {
    ...
    mapping(address => uint256[]) userStakeChangedBlockTime;
    ...
    modifier onPeriod() {
        require(
            stakePeriod.startTime <= block.timestamp &&
                block.timestamp < stakePeriod.periodFinish,
            "not stake period" // prefix
        );
        _;
    }
    ...
    function setPeriod(uint256 _startTime, uint256 _period) public onlyOwner {
    ...
    function stake(uint256 amount) public onPeriod {
        require(
            stakeToken.allowance(_msgSender(), address(this)) >= amount,
        ...
        emit Staked(
            _msgSender(),
            amount,
            userStakeAmountByBlockTime[sender][block.timestamp],
            block.timestamp
        );
    }
    ...
    function _updateStakeInfo(address sender, uint256 amount) private {
        ...
        if (historyLength == 0) {
            ...
            uint256 lastChangedBlockTime = userStakeChangedBlockTime[sender][
                historyLength.sub(1)
            ];
            uint256 lastStakedAmount = userStakeAmountByBlockTime[sender][
                lastChangedBlockTime
            ];
            userStakeChangedBlockTime[sender].push(block.timestamp);
            userStakeAmountByBlockTime[sender][
                block.timestamp
            ] = lastStakedAmount.add(amount);
        }
    }
    ...
    function unStake(uint256 amount) public {
        ...
        require(
            userStakeChangedBlockTime[_msgSender()].length > 0,
            "stake amount is 0"
        );
        address sender = _msgSender();
        ...
        uint256 stakedAmount = userStakeAmountByBlockTime[sender][
            lastChangedBlockTime
        ];
        require(
            stakedAmount >= amount,
        ...
        userStakeAmountByBlockTime[sender][blockTime] = stakedAmount.sub(
            amount
        );
        ...
        emit UnStaked(
            sender,
            amount,
            userStakeAmountByBlockTime[sender][blockTime],
            blockTime
        );
    }
    ...
    function isSatisfied(address user) external view override returns (bool) {
        ...
        uint256 endBlockTime = (stakePeriod.periodFinish > block.timestamp)
            ? block.timestamp
            : stakePeriod.periodFinish;

        for (uint8 i; i < userStakeChangedBlockTime[user].length; i++) {
            changedBlockTime = userStakeChangedBlockTime[user][i];
            changedStakeAmount = userStakeAmountByBlockTime[user][
                changedBlockTime
            ];
            satisfiedPeriod = _calcSatisfiedPeriod(
                stakeAmount,
                satisfiedPeriod,
                changedBlockTime.sub(prevBlockTime)
            );
        ...
        satisfiedPeriod = _calcSatisfiedPeriod(
            stakeAmount,
            satisfiedPeriod,
            endBlockTime.sub(changedBlockTime)
        );
    ...
    function _calcSatisfiedPeriod(
        ...
        return
            stakeAmount >= requiredStakeAmount
                ? satisfiedPeriod.add(retentionPeriod)
                : 0;
    }
}
